function alignedWaves = alignOnFallLinExact(waves)
% alignedWaves = alignOnFallLinExact(waves)
%
...
% For each time, get rid of the ones that come too late (minsI contains
...
% threshold or come too late
firstPostThreshAll = max(bsxfun(@times, greaterThanThresh, justCounting)) + 1;
...
threshPoints = alignedWaves(threshSamp, :);   % threshold has been crossed (so these are all below threshold)
...
% trim off one timepoint at the end (we've already effectively trimmed one
% at the front)
% alignedWaves = alignedWaves(1:end-1,:);
