function alignedWaves = alignOnRiseLin(waves)
% This function is deprecated. alignOnRiseLinExact should now always be
...
% let the wave come back up, since we're aligning on the upswing.
...
for w = 1:size(iWaves, 2)
  firstPostThresh = minsI(w) + find(iWaves(minsI(w)+1:end, w) > thresh, 1);
