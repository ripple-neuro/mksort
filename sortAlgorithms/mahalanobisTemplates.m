function [units, unitsDefined] = mahalanobisTemplates(Template, waves, waveStart)
% [units, unitsDefined] = mahalanobisTemplates(Template, waves, waveStart)
%
%
% Input values:
%
...
acceptanceFactor = 0.03;

unitsDefined = [];

% Figure out which units are defined
for u = 1:length(Template.params)
...
for u = 1:length(unitsDefined)
  ...
  scores = sqrt(sum((allScores(goodRows, :)).^2) / sum(goodRows));
  invScores = Template.params(unit+1).acceptance ./ (0.001 + scores);
  units(u, :) = invScores .* (scores <= Template.params(unit+1).acceptance * acceptanceFactor);
end
