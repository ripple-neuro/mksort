function draggableActPlusUp(h,varargin)
% draggableActPlusUp - Make it so that a graphics object can be dragged in
%   a figure. This function makes an object interactive by allowing it to
%   be dragged accross a set of axes, following or not certain constraints.
%   This allows for intuitive control elements which are not buttons or
%   other standard GUI objects, and which reside inside an axis. Typical
%   use involve markers on an axis, whose position alters the output of a
%   computation or display
% 
%   >> draggableActPlusUp(h);
%   
...
%   command.
%
%   >> draggableActPlusUp(h,...,motionfcn)
%
...
%
%   >> draggableActPlusUp(h,...,motionfcn, buttonupfcn)
%
%   where both "motionfcn" and "buttonupfcn" are function handles, does the
%   same as above but also executes buttonupfcn when the click is released.
%   "buttonupfcn" must be after "motionfcn".
%
%   >> draggableActPlusUp(h,...,constraint,p);
%
...
%   -----------------------------------------------------------------------
%
%   >> draggableActPlusUp(h,'off')
%
%   returns object h to its original, non-draggable state.
%
%   See the source code (e.g. by issuing "type draggableActPlusUp" at the Matlab 
%   prompt) for implementation notes.
%
% ==============================================================================
% This is a modified version of draggable.m from Mathworks Central, which has:
...
% ==============================================================================
%
% This file is included as part of the spike sorting software package MKort, 
...
%               every renderer.
% 2010-???      Modified by Matt Kaufman to include additional features.
%
...
%   on the graphics driver used and may differ from what is expected. 
...
% Modified by MK to allow clicking to switch active objects and to execute
% a user-specified function when releasing the mouse button
...
for k = 1:Noptarg
    ...
    if isa(current_arg,'function_handle') && nFcns == 0;
        user_movefcn = current_arg;
        nFcns = nFcns + 1;
    end;
    if isa(current_arg,'function_handle') && nFcns == 1;
        user_buttonupfcn = current_arg;
    end;
...
setappdata(h,'user_buttonupfcn',user_buttonupfcn);
...
% obj here is the figure containing the object
% MK CHANGE: was not resetting functions appropriately
set(obj,'WindowButtonMotionFcn','');
set(obj,'WindowButtonDownFcn',[]);
set(obj,'WindowButtonUpFcn',[]);

try
  user_buttonupfcn = getappdata(h,'user_buttonupfcn');
  if ~isempty(user_buttonupfcn)
    feval(user_buttonupfcn,h);
  end;
end
...
try
  ...
  warning('Error in draggableActPlusUp');
end
